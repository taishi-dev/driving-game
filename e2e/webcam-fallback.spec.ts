...
async function startFreeDrive(page: Page): Promise<void> {
  ...
  // with the button labelled フリーモード. (B11: the camera-denied overlay is
  // now localized, so the overlay assertions below use the Japanese strings.)
...
test("camera-denied shows the keyboard-fallback overlay", async ({ page }) => {
  ...
  // B11: the overlay is localized (visionStatus.ts); the page runs in Japanese.
...
// the car just short of the goal (the B7c `__driveDebug.teleport` sweep aid —
// zero-velocity chassis placement), then DRIVE the last stretch with the
// keyboard until the goal triggers success + feedback.
//
// B11 note: the driving screen now (faithfully to the original) mounts the
// vision layer even when the camera is denied, and its MediaPipe model
// loading/contexts cut headless-SwiftShader frame rates to a few FPS. The
// scene clamps the physics delta to 1/30 s (driveScene.ts), so below 30 FPS
// sim time runs slower than wall clock and a FULL 150 m real-time drive can no
// longer finish inside any sane headless budget (measured locally: 2 FPS ⇒
// ~15x slow-motion). Teleporting near the goal keeps this test's actual
// subject — goal detection -> success -> feedback -> replay frames — while
// making the wall clock FPS-independent. The full-length real-time keyboard
// drive to 100/100 is covered by the HEADED fake-webcam spec
// (webcam-vision.spec.ts), per the B11 brief's headed verification loop.
test("reaching a lesson goal triggers success + feedback (grading relocation)", async ({
  ...
  // Wait for the scene + debug hook (set once the Babylon scene resolves), then
  // let KeyboardControls attach its listeners.
  await page.waitForFunction(
  ...
  // 5 m deep — missions.ts), then drive the last stretch forward (the car
  // faces -z; ArrowUp = throttle) until the goal fires.
  await page.evaluate(() => {
