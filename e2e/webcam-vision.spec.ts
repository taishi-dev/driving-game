...
// P11 — fake-webcam verification of the vision layer. Chrome's fake media device
...
//
// HEADED on purpose (the P11 brief prescribes headed + fake stream): headless
// Chromium uses SwiftShader + the TFLite CPU (XNNPACK) delegate, which starves
// the render loop; the scene's physics delta clamp (productDriveScene) then makes
// sim time much slower than wall clock, so a real-time drive can never reach the
// goal. Headed runs on the real GPU (MediaPipe GPU delegate + normal frame rate).
test.use({
  headless: false,
  launchOptions: {
...
// reintroduce the SwiftShader slowdown — this spec is local-only. (The
// camera-DENIED path stays covered on CI by webcam-fallback.spec.ts.)
...
test("fake webcam: pipeline starts, preview UI renders, MediaPipe loads (both languages)", async ({
  ...
  // FPS with vision running (brief: record the readout for P12). Give the loop a
  // moment to settle after model load, then log what the badge shows.
  await page.waitForTimeout(3_000);
  const fpsText = await page.getByTestId("drive-fps").innerText();
  console.log(`[p11] FPS with vision running: ${fpsText}`);

  await page.screenshot({ path: "test-results/p11-drive-vision-en.png" });
  ...
  await page.screenshot({ path: "test-results/p11-drive-vision-ja.png" });
...
test("fake webcam + no detectable input: keyboard drives the straight lesson to 100/100", async ({
  ...
  // Physics sim time tracks wall clock only up to the per-frame delta clamp, so
  // keep generous headroom for slower frame rates.
  ...
  console.log(`[p11] straight-lesson feedback score = ${score}`);
  expect(score.trim().startsWith("100")).toBe(true);

  expect(await missionState(page)).toBe("success");

  await page.screenshot({ path: "test-results/p11-feedback-100.png" });
  expect(errors, `page errors: ${errors.join("\n")}`).toEqual([]);
});
