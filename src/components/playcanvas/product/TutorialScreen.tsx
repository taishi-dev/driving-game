 ...
 *     in P11. The store wiring (startCalibration / pedalState / steeringAngle) is
 *     IDENTICAL, so P11 only needs to mount the vision pipeline for the live
 *     steering (step 3) and foot (step 4) bars to move. The `startCalibration()`
 *     effect on step 4 is the wiring point.
 *   - Per-language punctuation: the three quoted phrases in step 1 are joined by a
 *     localized `welcomeSep` (、 in ja, ", " in en) instead of a hardcoded 、 baked
 *     into the JSX (the E1 defect that leaked a Japanese comma into English).
 */
...
const STRINGS = {
  ...
  en: {
    ...
    exampleLine2:
      "Pay attention to the hands turning the wheel and the feet pressing the accelerator and brake.",
...
export function TutorialScreen() {
  ...
  const startCalibration = useDrivingStore((s) => s.startCalibration);
  ...
  const pedalInputMode = useDrivingStore((s) => s.pedalInputMode);
  const setPedalInputMode = useDrivingStore((s) => s.setPedalInputMode);
  const language = useDrivingStore((s) => s.language);
  const t = STRINGS[language];

  const [step, setStep] = useState<1 | 2 | 3 | 4 | 5>(1);

  // Start calibration once step 4 is reached (original behavior). Without the P11
  // vision pipeline the stage simply stays "waiting_for_brake" (measuring).
  useEffect(() => {
  ...
  return (
    <div className="relative w-full h-full bg-slate-900 text-white overflow-hidden flex flex-col items-center justify-center">
      ...
      <div className="relative z-10 bg-slate-800/90 p-8 rounded-xl max-w-2xl w-full shadow-2xl border border-slate-700 backdrop-blur-sm max-h-full overflow-y-auto">
        ...
        {/* Step 1: Intro */}
        {step === 1 && (
          <div className="text-center" data-testid="tutorial-step-1">
        ...
        {step === 2 && (
          <div className="text-center" data-testid="tutorial-step-2">
        ...
        {/* Step 3: Steering */}
        {step === 3 && (
          <div className="text-center" data-testid="tutorial-step-3">
            ...
            <div className="bg-slate-900/50 p-6 rounded-lg mb-6 border border-slate-700">
              ...
              <div className="mt-2 font-mono text-xl font-bold">{steeringAngle.toFixed(2)}</div>
            </div>
          </div>
        )}

        {/* Step 4: Pedals / calibration */}
        {step === 4 && (
          <div className="text-center" data-testid="tutorial-step-4">
            <h2 className="text-2xl font-bold text-orange-400 mb-4">{t.pedalTitle}</h2>
            <p className="text-slate-300 mb-4">
              {t.pedalLine1}
              <br />
              {t.pedalLine2Pre}
              <span className="text-yellow-400 font-bold">{t.pedalLine2Highlight}</span>
              {t.pedalLine2Post}
            </p>
            ...
            {calibrationStage === "calibrated" && (
              <p className="text-green-400 font-bold animate-pulse">{t.calibratedMessage}</p>
            )}

            {/* Keyboard fallback (original semantics: switching also advances a step). */}
            {pedalInputMode === "keyboard" ? (
              <div className="mt-4 p-3 bg-cyan-900/30 border border-cyan-700 rounded">
                <p className="text-cyan-300 text-sm font-bold mb-2">{t.keyboardModeActive}</p>
                <button
              ...
              <button
                onClick={() => {
                  setPedalInputMode("keyboard");
                  nextStep();
                }}
                data-testid="tutorial-keyboard-mode"
        ...
        {/* Step 5: Done */}
        {step === 5 && (
          <div className="text-center" data-testid="tutorial-step-5">
