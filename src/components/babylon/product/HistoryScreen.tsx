...
import { getScoreRank, type ScoreRank } from "@/lib/missionLog";
import { LESSON_BRIEFINGS, FREE_MODE_TITLE, getLessonTitle, type Language } from "@/lib/lessonCatalog";
import type { LessonId } from "@/lib/store";
import { COMMON_STRINGS } from "@/lib/uiStrings";

/**
 * B10 — real driving-history screen for the Babylon product (replaces the
 * StubScreens placeholder). Ported from the original `ui/HistoryScreen.tsx`:
 * reads the last 10 records for the signed-in user from the Firestore
 * `mission_logs` collection (the SAME path the deployed owner-isolation rules
 * cover — the query filters on `userId == user.uid`, matching the rules'
 * owner check), caches them in the store's `missionHistory`, and renders the
 * score/rank/time cards.
 *
 ...
 *
 * Lesson display names come from the pure lessonCatalog (bilingual for ALL
...
/** Rank badge colors keyed by the pure missionLog rank. */
  ...
  useEffect(() => {
    ...
    async function fetchHistory() {
      ...
      try {
        ...
        // an error when there is nothing cached to display. Store an error KIND,
        // not localized text, so a later language switch re-localizes it.
