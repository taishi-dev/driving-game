 ...
 * step-4 calibration/keyboard-fallback semantics). Differences from the original,
 * both deliberate for the Babylon port:
 *   - No VisionController camera background — MediaPipe vision arrives in B11;
 *     the store wiring (startCalibration / pedalState / steeringAngle) is
 *     identical, so B11 only needs to mount the vision pipeline for the live
 *     bars to move.
 *   - No R3F TutorialPlainScene import; step 2 keeps the original example video
 *     (/videos/tutorial.mp4), which is what the original step 2 showed.
 */
...
export function TutorialScreen() {
  ...
  // Start calibration once step 4 is reached (original behavior; without the
  // B11 vision pipeline the stage simply stays "waiting_for_brake").
  useEffect(() => {
