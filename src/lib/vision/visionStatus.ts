...
import type { CalibrationStage } from "./pedalDecision.ts";

/**
 * B11 — pure, localized presentation of the vision status panel + camera-error
 * overlay. Extracted from the original VisionController's inline (English-only)
 * `getStatusDisplay` / `getUserMedia` error strings so the Babylon product can
 * render the same status in BOTH languages. Pure + `node --test`-covered; the
 * product `VisionController` maps the returned `tone` to colors and applies the
 * text. No store / DOM / clock.
 */
